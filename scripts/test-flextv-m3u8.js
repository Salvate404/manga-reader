...
(async () => {
  ...
  // list endpoint?
  const js = require("fs").readFileSync("scripts/_flextv_Dw4UPhha.js", "utf8");
  const webs = [...js.matchAll(/"(\/web[A-Za-z0-9_\/]+)"/g)].map((m) => m[1]);
  console.log("webs", [...new Set(webs)]);

  // section content list might have episodes
  const section = await apiGet("/webGetSeriesSectionContentList", {
    series_id: "qLZmA8AZvr",
  });
  console.log("section", JSON.stringify(section).slice(0, 800));
})();
